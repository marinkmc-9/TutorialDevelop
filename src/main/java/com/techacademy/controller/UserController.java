...
public class UserController {
    ...
    public String getUser(@PathVariable("id") Integer id, Model model, User user) { //User を追加

        // idがnullではない場合、サービスから取得した値をセットする
        if(id!=null) {
            // Modelに登録
            ...
            // idがnullの場合
            model.addAttribute("user",user);//必ず型を揃えること（user)
        }
        // 更新画面に遷移
        return "user/update";
    }
    ...
    public String postUser(@Validated User user, BindingResult res, Model model) {
        if(res.hasErrors()) {
            // エラーあり
            return getUser(null,model,user); //必ず型を揃えること（Idはnull,model,user)
        }
        // User登録
        service.saveUser(user);
        // 一覧画面にリダイレクト
